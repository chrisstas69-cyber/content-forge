...
import { Loader2, MessageCircle, RefreshCw, Check, X, AlertCircle, Users, Mic } from 'lucide-react'
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">AI generates replies in your voice. Approve or edit before posting.</p>
        </div>
        <button onClick={fetchNew} disabled={fetching} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-orange-500 text-white text-sm font-medium disabled:opacity-50">
      ...
      <div className="flex gap-2">
        {filters.map(f => (
          <button key={f.id} onClick={() => setFilter(f.id)} className={`px-3 py-1.5 rounded-md text-xs font-medium ${filter === f.id ? 'bg-neutral-900 text-white dark:bg-white dark:text-neutral-900' : 'border border-neutral-200 dark:border-neutral-800'}`}>
            {f.label}
          </button>
        ))}
      </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${c.replyStatus === 'replied' ? 'bg-emerald-100 text-emerald-700' : c.replyStatus === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-neutral-100 text-neutral-500'}`}>{c.replyStatus}</span>
                  </div>
                  ...
                  {c.replyText && (
                    <div className="mt-2 p-3 rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900">
                      ...
                      {editingReply === c.id ? (
                        <div>
                          <textarea value={customReply} onChange={e => setCustomReply(e.target.value)} rows={2} className="w-full px-2 py-1 rounded border border-blue-200 dark:border-blue-800 bg-white dark:bg-neutral-900 text-sm" />
                  ...
                  {c.replyStatus === 'pending' && c.replyText && editingReply !== c.id && (
                    <div className="flex gap-2 mt-2">
                      <button onClick={() => approve(c.id)} className="inline-flex items-center gap-1 px-3 py-1 text-xs rounded-md bg-emerald-500 text-white"><Check className="size-3" /> Approve & Post</button>
                      <button onClick={() => { setEditingReply(c.id); setCustomReply(c.replyText) }} className="inline-flex items-center gap-1 px-3 py-1 text-xs rounded-md border border-neutral-200 dark:border-neutral-800">Edit</button>
                      <button onClick={() => ignore(c.id)} className="inline-flex items-center gap-1 px-3 py-1 text-xs rounded-md text-neutral-500"><X className="size-3" /> Ignore</button>
                    </div>
                  )}
                </div>
              </div>
...
export function VoiceProfileSettings() {
  ...
  const [form, setForm] = useState({
    persona: '', tone: '', signaturePhrases: '', avoidPhrases: '',
    autoReplyMode: 'suggest', replyLength: 'short',
  })
  ...
  async function save() {
    setSaving(true)
    try {
      await fetch('/api/voice-profile', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500 mt-1">Defines your AI persona for comment replies, voiceover scripts, and all AI-generated content.</p>
      </div>
...
export function Competitors() {
  ...
  async function addCompetitor() {
    ...
    try {
      const res = await fetch('/api/competitors', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ platform: newPlatform, handle: newHandle.replace('@', '') }) })
      if (!res.ok) throw new Error('Failed')
  ...
  async function refresh() {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['competitors'] })
    } catch (err: any) { toast.error(err.message) } finally { setRefreshing(false) }
  }
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">Track competitors and get alerts when their posts go viral.</p>
        </div>
      ...
      {competitors.length === 0 && alerts.length === 0 && !isLoading && (
        <div className="text-center py-16 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800">
          <Users className="size-12 mx-auto text-neutral-300 mb-3" />
